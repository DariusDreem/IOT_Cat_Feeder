...
export function updateWsUrl(url: string) {
  if (url && url.startsWith('ws')) {
    // Si l'utilisateur met l'IP sans le port ou avec le port 8080 sans /ws, on corrige pour lui
    let finalUrl = url
    if (finalUrl.endsWith(':8080')) {
      finalUrl = finalUrl + '/ws'
    }
    // S'assurer qu'il n'y a pas un double slash de fin
    if (finalUrl.endsWith('//ws')) {
        finalUrl = finalUrl.replace(/\/\/ws$/, '/ws')
    }
    RASPBERRY_WS_URL = finalUrl
  }
}
